...
@section('breadcrumb')
	...
	<li class="breadcrumb-item active" aria-current="page">Tambah Suku Cadang</li>
@endsection
	...
	<div class="container-fluid">
		<div class="card shadow-sm border-0 mb-4 p-3">
			<div class="card-header bg-white border-0 d-flex justify-content-between align-items-center">
				<h5 class="mb-0 fw-semibold fs-4">Tambah Suku Cadang</h5>
			...
			<div class="card-body">
				<form action="{{ route('parts.store') }}" method="POST" enctype="multipart/form-data">
					...
					<div class="mb-3">
						...
						<textarea name="description" class="form-control @error('description') is-invalid @enderror" rows="3" placeholder="Deskripsi singkat tentang suku cadang ini">{{ old('description') }}</textarea>
						@error('description')
							<div class="invalid-feedback d-block">{{ $message }}</div>
						@enderror
					</div>
...
@push('scripts')
	<script>
		const partForm = document.querySelector('form[action="{{ route('parts.store') }}"]');
